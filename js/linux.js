...
// Linux 로고 애니메이션
function initLinuxLogo() {
    const logo = document.querySelector('.linux-logo');
    if (!logo) return;
    ...
    logo.addEventListener('click', function() {
        ...
        setTimeout(() => {
            this.style.animation = 'linuxBounce 0.5s ease-in-out 3';
        }, 10);
    });
...
const LinuxUtils = {
    // 랜덤 Linux 색상 생성
    getRandomLinuxColor: function() {
        const colors = ['#0066cc', '#004499', '#3399ff', '#ff6600'];
...
// 전역 변수로 유틸리티 노출
window.LinuxUtils = LinuxUtils;

// 성능 최적화를 위한 디바운스 적용
window.addEventListener('scroll', LinuxUtils.debounce(animateOnScroll, 10));
window.addEventListener('resize', LinuxUtils.debounce(() => {
    // 리사이즈 시 필요한 처리
